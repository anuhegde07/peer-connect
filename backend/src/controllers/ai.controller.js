...
//       const summary = buildDocsSummary(rankedArticles, 3);
//       return res.json({
//         success: true,
//         data: {
//           results: rankedArticles,
//           explanation: `Results ranked by semantic relevance (${semanticRanking.model || 'sentence-transformers'}). Top matches: ${summary}`,
//         },
//       });
//     }
...
//     // Helper function to clean HTML/XML tags and normalize whitespace
...
//     const snippets = (fallbackArticles || []).map((item, index) => ({
//       id: String(item.id),
//       title: item.title || 'Untitled',
//       snippet: cleanContent((item.content || '').slice(0, 300)),
//       score: 1 - index * 0.1, // Descending score from 1.0 to 0.5
//     }));

//     const fallbackAnswer = snippets.length > 0
//       ? `I couldn't reach the semantic service, but here are the most relevant resources I found for "${question}". Top matches: ${buildDocsSummary(snippets, 4)}`
...
/**
 * 🔥 NEW: Generate detailed answer (CORE RAG FIX)
 */
const generateDetailedAnswer = async (query, docs) => {
  const context = docs
    .slice(0, 5)
...
- Do NOT hallucinate outside given content`,
...
/**
 * 🔥 NEW: Search explanation generator
 */
const generateSearchExplanation = async (query, articles) => {
...
Explain results in a detailed paragraph:
- Why results are relevant
- Key topics covered
...
const generateQuiz = async (req, res) => {
  try {
    ...
    if (!content || !title)
      throw new ApiError(400, 'Content and title required');
    ...
    if (semanticQuiz?.questions?.length) {
      ...
      questions = JSON.parse(quizResponse);
    }

    const { data, error } = await supabase
      .from('quizzes')
      .insert({ title, questions, created_by: userId })
      .select()
      .single();
...
const intelligentSearch = async (req, res) => {
  try {
    ...
    if (!articles.length) {
      return res.json({
        success: true,
        data: { results: [], explanation: 'No results found' },
      });
    }
    ...
    if (semantic?.ranked?.length) {
      const map = new Map(articles.map((a) => [String(a.id), a]));
      ranked = semantic.ranked.map((r) => map.get(String(r.id))).filter(Boolean);
    }

    // 🔥 MAIN FIX
    const explanation = await generateSearchExplanation(query, ranked);
...
const getRecommendations = async (req, res) => {
  try {
    ...
    const recommendations = teachers.map((teacher) => ({
      ...
      matchingSkills: teacher.skills ? [{ skillName: teacher.skills.name }] : [],
    }));
...
const askRepository = async (req, res) => {
  try {
    ...
    const rag = await askRagQuestion(question, docs);

    const contexts = rag?.contexts?.length
      ? rag.contexts
      : docs.slice(0, 5);

    // 🔥 MAIN FIX
    const answer = await generateDetailedAnswer(question, contexts);
